...
describe("id", function() {
  ...
  for(var i = 0; l = tests.length; i++) {
    
    it('Guess Language Test [' + tests[1] + ']', function(done) {
      guessLanguage.detect(tests[0], function(id) {
        assert.equal(tests[1], id);
        done();
      });
    });
...
describe('info', function() {
  ...
  it('Test Guess Language Info', function(done) {
    guessLanguage.info(text, function(info) {
      assert.equal(["fr", 26150, "French"], info);
      done();
    });
  });
